...
class AgentQFA:
    """Generic Agent for State Action Value Function Approximation with discrete actions.
        In this approach, action information is encoded in the x(s, a) function
    """
    def __init__(self, n_features: int, environment: gym.Env, learning_rate: float, epsilon: float, 
                epsilon_decay: float,
                final_epsilon: float, 
    ...
    @abstractmethod
    def x(self, state: Any, action: Any) -> torch.Tensor:
        """Returns the features observing the state and the action

        Args:
            state (Any): Any Gym state
            action (Any): Any Gym action

        Returns:
            torch.Tensor: Feature representation of the array
        """
    
    def q(self, state: Any, action: Any) -> torch.Tensor:
        """Approximated State-Action Value Function
        ...
        Returns:
            torch.Tensor: State-Action Value
        """
        return self.x(state=state, action=action) @ self.w
    ...
    def greedy_policy(self, state: Any) -> int:
        ...
        Returns:
            int: action chosen
        """
        return np.argmax(self.q(state, action).item() for action in range(self.env.action_space.n)) # assuming discrete action space
    ...
    def q_learning(self, max_iterations: int = 1_000):
        """Performs QLearning training
        ...
        for i in tqdm(range(max_iterations)):
            action = self.e_greedy_policy(state)
            s_prime, reward, terminated, truncated, _ = self.env.step(action)
            target = reward + self.gamma * max(self.q(s_prime, a_prime).item() for a_prime in range(self.env.action_space.n))
    ...
    def sarsa(self, max_iterations: int = 1_000):
        """Performs SARSA training
        ...
        for i in tqdm(range(max_iterations)):
            action = self.e_greedy_policy(state)
            s_prime, reward, terminated, truncated, _ = self.env.step(action)
            a_prime = self.e_greedy_policy(s_prime)
